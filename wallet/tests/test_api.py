...
class WalletAPITests(APITestCase):
    ...
    def test_get_wallet_list(self):
        response = self.client.get(self.wallets_url, HTTP_ACCEPT="application/vnd.api+json")
    ...
    def test_sort_wallets_by_balance(self):
        Transaction.objects.create(wallet=self.wallet1, txid="tx1", amount=Decimal("50.00"))
        Transaction.objects.create(wallet=self.wallet2, txid="tx2", amount=Decimal("100.00"))

        response = self.client.get(f"{self.wallets_url}?sort=balance", HTTP_ACCEPT="application/vnd.api+json")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        raw = json.loads(response.content.decode('utf-8'))
        wallets = raw["data"]
        self.assertGreater(Decimal(wallets[1]['attributes']['balance']), Decimal(wallets[0]['attributes']['balance']))
...
class TransactionAPITests(APITestCase):
    ...
    def test_create_valid_transaction(self):
        ...
        data = {
            "data": {
                ...
                "relationships": {
                    "wallet": {
                        "data": {"type": "Wallet", "id": str(self.wallet.id)}
                    }
                }
            }
        }
    ...
    def test_create_transaction_negative_balance(self):
        Transaction.objects.create(wallet=self.wallet, txid="tx_initial", amount=Decimal("50.00"))
    ...
    def test_search_transaction_by_txid(self):
        Transaction.objects.create(wallet=self.wallet, txid="unique_txid", amount=Decimal("100.00"))
        response = self.client.get(f"{self.transactions_url}?filter[txid]=unique_txid",
                                   HTTP_ACCEPT="application/vnd.api+json")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        raw = json.loads(response.content.decode('utf-8'))
    ...
    def test_pagination(self):
        for i in range(15):
            Transaction.objects.create(wallet=self.wallet, txid=f"tx{i}", amount=Decimal("10.00"))

        response = self.client.get(f"{self.transactions_url}?page[size]=10", HTTP_ACCEPT="application/vnd.api+json")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        raw = json.loads(response.content.decode('utf-8'))
        self.assertEqual(len(raw["data"]), 10)  # Должно быть 10 записей на первой странице
